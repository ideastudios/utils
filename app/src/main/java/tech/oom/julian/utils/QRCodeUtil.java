...
//            hints.put(EncodeHintType.MARGIN, 2); //default is 4
    ...
    public static void showThreadImage(final Activity mContext, final String text, final ImageView imageView, final int centerPhoto) {
        String preContent = SharedPreferencesUtil.getInstance().getString("share_code_content","");
        if (text.equals(preContent)) {
            String preFilePath = SharedPreferencesUtil.getInstance().getString("share_code_filePath","");
            imageView.setImageBitmap(BitmapFactory.decodeFile(preFilePath));

        } else {
            SharedPreferencesUtil.getInstance().putString("share_code_content",text);
            final String filePath = getFileRoot(mContext) + File.separator + "qr_" + System.currentTimeMillis() + ".jpg";
            SharedPreferencesUtil.getInstance().putString("share_code_filePath",filePath);
