...
public class MyExtAudioRecorder {
     ...
     * this size is written to the header/data chunk in the wave file */
     ...
     */
    public MyExtAudioRecorder(boolean uncompressed, int audioSource, int sampleRate, int channelConfig, int audioFormat) {
        try {
    ...
    public static MyExtAudioRecorder getInstanse(Boolean recordingCompressed) {
        MyExtAudioRecorder result = null;

        if (recordingCompressed) {
            result = new MyExtAudioRecorder(false, AudioSource.MIC, sampleRates[3], AudioFormat.CHANNEL_IN_MONO, AudioFormat.ENCODING_PCM_16BIT);
        } else {
            int i = 0;
            do {
                result = new MyExtAudioRecorder(true, AudioSource.MIC, sampleRates[i], AudioFormat.CHANNEL_IN_MONO, AudioFormat.ENCODING_PCM_16BIT);

            }
            while ((++i < sampleRates.length) & !(result.getState() == MyExtAudioRecorder.State.INITIALIZING));
        }
        return result;
    }
